...
void Draw(void)
{
	...
	// 绘制第二个黑色的球（地球）
	xRot += (float)0.01f;  // 地球的自转速度
	float earthOrbitRadius = 4.0f;  // 地球绕太阳的轨道半径
	float earthOrbitSpeed = 0.3f;  // 增加地球绕太阳的旋转速度
	vmath::mat4 earthOrbit = vmath::rotate(xRot * earthOrbitSpeed, vmath::vec3(0.0, 1.0, 0.0)) * vmath::translate(earthOrbitRadius, 0.0f, 0.0f);
	vmath::mat4 trans2 = vmath::perspective(60, aspact, 1.0f, 500.0f) * vmath::translate(0.0f, 0.0f, -5.0f) * earthOrbit * vmath::scale(0.3f);
	...
	// 绘制第三个球（月球）
	xRot += (float)0.005f;  // 月球自转速度
	float moonOrbitRadius = 1.0f;  // 月球绕地球的轨道半径
	float moonOrbitSpeed = 2.0f;  // 控制月球绕地球的旋转速度
	vmath::mat4 moonOrbit = vmath::rotate(xRot * moonOrbitSpeed, vmath::vec3(0.0, 1.0, 0.0)) * vmath::translate(moonOrbitRadius, 0.0f, 0.0f);

	// 月球绕地球的变换
	vmath::mat4 trans3 = trans2 * moonOrbit * vmath::rotate(xRot, vmath::vec3(0.0, 1.0, 0.0)) * vmath::translate(1.0f, 0.0f, 0.0f) * vmath::scale(0.1f);
	GLfloat vColor3[4] = { 0.0f, 0.0f, 1.0f, 1.0f };  // 月球的颜色
	// 绘制月球
